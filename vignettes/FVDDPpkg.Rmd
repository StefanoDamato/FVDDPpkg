...
Before starting, it should be mentioned that the sole purpose of this paper is to provide intuitive and easily replicable instructions on how to use the FVDDPpkg package on \textsf{R}. For this reason, the underlying theory will not be developed except in the minimum necessary terms; for more information, please refer to the bibliography cited here.
...
* $M$ is a set of multiplicities; its elements are $K$-dimensional integer vectors in the form $\bfm = (m_1, \dots, m_K)$. Intuitively, they can be thought as their $j$-th entry counts the occurences of the $j$-th unique value, $y_j^\ast$.
...
$\alpha = \theta P_0$, where $\theta$ is the intensity, and the probability measure $P_0$ gives the centering. For a wide review of the Dirichlet Process and its properties, refer to @GhoshalVanDerVaart2017.
* $\delta_y$ is a Dirac measure who puts mass on the point $y$.

The derivation of this model, which stems from the study of population genetics, is done by exploiting the concept of duality for Markov processes (@PapaspiliopoulosRuggiero2014) by applying it to the results of @EthierGriffiths1993, among the others, from the seminal work of @FlemingViot1979.
...
* $M$ is stored as an integer matrix `M` of size $|M| \times K$, where $|M|$ denotes the cardinality of $M$ and $K$ is the length of $\mathbf{y}^\ast$. Each vector $\bfm$ is stored as a row of `M`.
...
$$w_{\bfm}^\phi \propto w_\bfm\mathrm{PU}(\bfn |\bfm)$$ where $\mathrm{PU}(\bfm |\bfn)$ denoes the probability of drawing a vector of multiplicities $\bfn$ starting from $\bfm$ via Polya urn sampling scheme under $\theta$ and $P_0$ specified by the model, and
...
To do so, it has been shown by @AscolaniLijoiRuggiero2023 that it is necessary to create two processes. The first has to be propagated forward from $t_0$ to $t-{i-1}$ as in the previous sections; the second one has to be run backward using the same strategy: initialize and update it at $t_n$ and propagate it towards $t_{n-1}$ (with a positive time $t_n - t_{n-1}$ in the function), and sequentially update and propagate until $t_{i+1}$ is reached.

Doing this, one will get that 
$$X_{t_{i-1}} \ |\ Y_0, \dots, Y_{i-1} \sim \sum_{\bfn_{i-1} \in M_{i-1}} v_{\bfn_{i-1}} \Pi_{\alpha + \sum_{j=1}^K n_{i-1,j}\delta_{y_j^\ast}}$$
and $$X_{t_{i+1}} \ |\ Y_{i+1}, \dots, Y_{n}=  \sum_{\bfnfuture \in M_{i+1}} v_{\bfnfuture} \Pi_{\alpha + \sum_{j=1}^K n_{i+1,j}\delta_{y_j^\ast}}$$
where the subscript $i-1$ and $i+1$ are necessary to indicate elements from the past or the future mixture, $v$ stands for the weights and for example $n_{i-1, j}$ is the $j$-th component of the etor $\bfnpast$ (same for $\bfnfuture$). 

Provided this decription based on available data from past and future, call $\bfn_i$ the multiplicities generated by the vector $Y_i$. Then

$$X_{t_i} \ |\ X_{t_{i-1}}, X_{t_{i+1}}, Y_i \sim  \sum_{\substack{\bfnpast \\ \in M_{i-1}}}\sum_{\substack{\bfnfuture \\ \in M_{i+1}}}  v_{\bfnpast} v_{\bfnfuture} \sum_{\substack{(\bfkpast, \bfkfuture) \\ \in \mathcal{D}^{\bfnpast, \bfnfuture}}} w_{\bfkpast, \bfn_i, \bfkfuture}^{\bfnpast, \bfnfuture} \Pi_{\alpha  + \sum_{j=1}^K (\bfk_{i-1, j} + \bfn_{i,j} + \bfk_{i+1, j} )\delta_{y_j^\ast}}$$
where:

* if $P_0$ is non-atomic, define the sets
$$ \mathcal{D}_{i-1} := \{ j \in \{ 1, \dots, K\} : n_{i-1, j} > 0 \ \text{and either} \ n_{i,j}>0 \ \text{or} \  n_{i+1,j}>0 \},$$
$$\mathcal{D}_{i+1} := \{ j \in \{ 1, \dots, K\} : n_{i+1, j} > 0 \ \text{and either} \ n_{i,j}>0 \ \text{or} \  n_{i-1,j}>0 \}$$
and $$
\mathcal{S} := \mathcal{D}_{i-1} \cup \mathcal{D}_{i+1}$$
to express the indices of shared values among different times. Then
$$\begin{align*} \mathcal{D}^{\bfnpast,\bfnfuture} = \{ (\bfkpast, \bfkfuture) : &\bfkpast \leq \bfnpast \ \text{and} \ k_{i-1, j} > 0 \ \forall \ j \in \mathcal{D}_{i-1},\\
    &\bfkfuture \leq \bfnfuture \ \text{and}  \ k_{i+1, j} > 0 \ \forall \ j \in \mathcal{D}_{i+1} \} \end{align*}$$
    ...
    * if $\mathcal{S} \neq \emptyset$:
$$w_{\bfkpast, \bfn_i, \bfkfuture}^{\bfnpast, \bfnfuture} \ \propto \tilde{p}_{\bfkpast, \bfkfuture}^{\bfnpast, \bfnfuture} \frac{\theta^{(|\bfkpast|)} \theta^{(|\bfkfuture|)}}{(|\theta + |\bfn_i|)^{(|\bfkpast|+|\bfkfuture|)}} \\ \times \prod_{j \in \mathcal{S}}\frac{(k_{i-1, j} + n_{i,j} + k_{i+1,j}-1)!}{(k_{i-1,j}-1)! (n_{i,j}-1)! (k_{i-1,j}-1)!} $$
if $(\bfkpast, \bfkfuture) \in \mathcal{D}$, and $0$ otherwise, under the convention that $(-1)!=1$.

* if $P_0$ is atomic, let $$\mathcal{D}^{\bfnpast, \bfnfuture}:=\{ (\bfkpast, \bfkfuture) : \bfkpast \leq \bfnpast, \bfkfuture \leq \bfnfuture\}$$
...
```
In the example above, two process we created with $\theta = 0.7$ and $P_0 \sim \mathrm{Beta}(4, 7)$. The signal was updated once in the past, and twice in the future (with a propagation between the two updates).
```{r, results='hide'}
...
```
As before, the mixture referred to pas observations is updated once, the one referred to future observations is updated twice.
```{r, results='hide'}
...
FVDDP_SMOOTH_ATOMIC
```
In this case, the mixture is clearly bigger. The reason is that when $P_0$ is atomic, the set $\mathcal{D}^{\bfnpast, \bfnfuture}$ does not put constraints based on the appearance of shared values across diffferent times. 
...
```
In the previous chunk, a process with hyperparameters $\theta = 3$ and $P_0 \sim \mathcal{N}(-1, 3)$. The syntax of the approximating functions is just the same as in `propagate()`, with the exceptions that one must specify the number of samples `N` to be drawn.
...
FVDDP_APPR_PROP
```
The results is agin an `fvddp` object. In order to measure the accuracy of such approximation, one has to compute the exact output of the propagation, again with time $t=0.45$.
```{r, results='hide'}
...
In the following example, one can see how to copy the one done in the exact smoothing
```{r, results='hide'}
...
Another tool to cut the computational cost of predictive of smoothing inference is given by the `prune()` function. It allows to remove from the mixture all vectors $\bfm$ whose weight $w_\bfm$ is under some treshold $\varepsilon$. Such eights are then normalized such taht their sum is $1$.
...
This is implemented via `predictive.struct()`; the arguments are the same as in `posterior.sample()`. 
```{r, results = 'hide'}
