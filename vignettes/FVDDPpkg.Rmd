...
$$X_{t_i} \ |\ X_{t_{i-1}}, X_{t_{i+1}}, Y_i \sim  \sum_{\substack{\bfnpast \\ \in M_{i-1}}}\sum_{\substack{\bfnfuture \\ \in M_{i+1}}}  u_{\bfnpast} v_{\bfnfuture} \sum_{\substack{(\bfkpast, \bfkfuture) \\ \in \mathcal{D}^{\bfnpast, \bfnfuture}}} w_{\bfkpast, \bfn_i, \bfkfuture}^{\bfnpast, \bfnfuture} \Pi_{\alpha  + \sum_{j=1}^K (\bfk_{i-1, j} + \bfn_{i,j} + \bfk_{i+1, j} )\delta_{y_j^\ast}}$$
where:
...
FVDDP_SMOOTH_ATOMIC
```
In this case, the mixture is clearly bigger. The reason is that when $P_0$ is atomic, the set $\mathcal{D}^{\bfnpast, \bfnfuture}$ does not put constraints based on the appearance of shared values across different times. 
